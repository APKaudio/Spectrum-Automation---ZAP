#/ Updated ScanV10.8.2.py with new plot options, auto-open functionality, and enhanced scan_bands
#few little things - The generate plot Average button should work anytime. If it cant work anytime, it should be able to work if the scan is paused -
#The frequency shift should be a slide bar with the values of zero (default) 500, 1000, 5000, 10000
#The Scan RBW should be a text box AND a slider to set the value - the RBW should be default 10k but the range should be 5000, 10000, 25000,50000, 100000
#the cycle hold time should be default 3. but the scale should be .5 -10 in .5 second incriments
#the cycle wait time should also be a slider bar - default is 0 but it should be 0 to 600 seconds
...
DEFAULT_FREQ_SHIFT_HZ = 0 # New: Default Frequency Shift value (changed to 0)
...
def scan_bands(app_instance, csv_writer, selected_bands, scan_rbw_segmentation, rbw_config_val, vbw_config_val, max_hold_time, current_freq_offset, last_scanned_band_index=0):
    """
        ...
        while current_segment_start_freq_hz < band_stop_freq_hz:
         ...
         #   query_safe(app_instance.inst, "*OPC?") # Wait for the sweep to complete
            ...
            try:
                ...
                if app_instance.instrument_model == "N9340B":
                    trace_data_str = query_safe(app_instance.inst, ":TRAC2:DATA?")
                ...
                for j, amp_value in enumerate(trace_data):
                    ...
                    csv_writer.writerow([
                        f"{current_freq_for_point_hz / MHZ_TO_HZ:.3f}",  # Frequency in MHz for CSV
                        f"{amp_value:.3f}"
                    ])
...
class App(tk.Tk):
    def __init__(self):
        ...
        # New variables for RBW shifting
        self.scan_cycle_count = 0 # Tracks how many full scan cycles have completed
        self.current_freq_offset = 0 # The current frequency offset applied
        ...
        self.desired_max_hold_time_var = tk.DoubleVar(self, value=DEFAULT_MAXHOLD_TIME_SECONDS) # Changed to DoubleVar for slider
        ...
        self.desired_cycle_wait_time_var = tk.DoubleVar(self, value=DEFAULT_CYCLE_WAIT_TIME_SECONDS) # Changed to DoubleVar for slider
        ...
        self.desired_scan_rbw_segmentation_var = tk.DoubleVar(self, value=DEFAULT_SCAN_RBW_HZ) # Changed to DoubleVar for slider
        # New variable for Frequency Shift
        self.shift_freq_var = tk.DoubleVar(self, value=DEFAULT_FREQ_SHIFT_HZ) # Changed to DoubleVar for slider

        # NEW: Tkinter variables for slider indices (for discrete value sliders)
        self.rbw_values = [5000, 10000, 25000, 50000, 100000]
        self.rbw_val_to_idx = {val: i for i, val in enumerate(self.rbw_values)}
        self.rbw_slider_index_var = tk.IntVar(self, value=self.rbw_val_to_idx.get(DEFAULT_SCAN_RBW_HZ, 0)) # Default to index of 10000, or 0 if not found
        ...
        self.freq_shift_slider_index_var = tk.IntVar(self, value=self.freq_shift_val_to_idx.get(DEFAULT_FREQ_SHIFT_HZ, 0)) # Default to index of 0, or 0 if not found
    ...
    # NEW: Function to update scan RBW value from slider index
    ...
    # NEW: Function to update frequency shift value from slider index
    ...
    def create_widgets(self):
        ...
        row_idx += 1
        # Replaced "Display Scale (LOG/LIN)" with "High Sensitivity"
        ...
        # The original preamp checkbox is now redundant if High Sensitivity controls it.
        # However, the user's request implies "High Sensitivity" *is* the preamp control.
        # So, I'll remove the old "Preamplifier (ON/OFF)" and use "High Sensitivity" for it.
        # I will keep the `desired_preamp_var` in `__init__` for now, but it won't be tied to a UI element.
        # If `high_sensitivity_var` is intended to replace `desired_preamp_var` entirely,
        # then `desired_preamp_var` can be removed from `__init__` and `initialize_instrument`
        # should directly use `high_sensitivity_on`.
        # For now, I'll assume `high_sensitivity_var` is the new control for the preamplifier.
        # So, the `desired_preamp_var` and its UI element will be removed.
        ...
        # New "Scan RBW" for segmentation (this was already present, just ensuring row_idx is correct)
        row_idx += 1 # This will be the next available row after the new "High Sensitivity" checkbox
        tk.Label(scan_settings_frame, text="Scan RBW (Hz):", bg="black", fg="white").grid(row=row_idx, column=0, sticky=tk.W, pady=2)
        
        # Scan RBW Entry and Slider
        rbw_input_frame = tk.Frame(scan_settings_frame, bg="black")
        rbw_input_frame.grid(row=row_idx, column=1, sticky=tk.EW, pady=2)
        rbw_input_frame.grid_columnconfigure(0, weight=1) # Entry
        rbw_input_frame.grid_columnconfigure(1, weight=2) # Slider
        ...
        # Frequency Shift Entry and Slider Frame
        ...
        freq_shift_input_frame.grid_columnconfigure(0, weight=1) # Entry
        freq_shift_input_frame.grid_columnconfigure(1, weight=2) # Slider
        ...
        # Function to update slider from variable
        ...
        self.freq_shift_slider = tk.Scale(freq_shift_input_frame, 
                                          variable=self.freq_shift_slider_index_var, # Bind to index variable
        ...
        self.shift_freq_var.trace_add("write", update_freq_shift_from_var) # Trace variable to update slider


        row_idx += 1
        tk.Label(scan_settings_frame, text="Cycle Hold Time (s):", bg="black", fg="white").grid(row=row_idx, column=0, sticky=tk.W, pady=2)
        
        # Cycle Hold Time Slider
        ...
        self.cycle_hold_time_slider.set(DEFAULT_MAXHOLD_TIME_SECONDS) # Set default value
        ...
        self.cycle_wait_time_slider.set(DEFAULT_CYCLE_WAIT_TIME_SECONDS) # Set default value
    ...
    def connect_instrument(self):
        ...
        try:
            ...
            if instrument_id:
                ...
                high_sensitivity_on = self.high_sensitivity_var.get() # Get high sensitivity state from GUI
                preamp_on = self.desired_preamp_var.get() # Get high sensitivity state from GUI
    ...
    def query_instrument_settings(self):
        ...
        try:
            ...
            # Query the preamplifier state (High Sensitivity)
            print(f"  Preamplifier (High Sensitivity ON/OFF): {query_safe(self.inst, ':POWer:GAIN?')}")
            # Display scale is now always logarithmic, no need to query
    ...
    def start_scan_thread(self):
        ...
        # Get the new "Scan RBW" for segmentation (this remains constant)
        scan_rbw_segmentation = float(self.desired_scan_rbw_segmentation_var.get())
        # Get the frequency shift value
        ...
        # Reset cycle count and current frequency offset for a new scan thread
    ...
    def _run_scan(self, selected_bands, scan_rbw_segmentation, freq_shift_value, rbw_config_val, vbw_config_val, max_hold_time):
        ...
        try:
            ...
            while self.scanning:
                ...
                offset_str = f"Offset{int(self.current_freq_offset)}" # New: Offset for filename

                # Date and time for filename
                datetime_str = datetime.now().strftime("%Y%m%d_%H%M%S") # Added seconds for uniqueness
                
                # CSV filename using the new convention
                csv_filename = f"{scan_name}_{rbw_str}_{hold_str}_{offset_str}_{datetime_str}.csv"
                ...
                # Derive HTML filename from CSV filename for single scan plot
                html_filename_base = os.path.splitext(csv_filename)[0] # Get name without .csv
                ...
                try:
                    with open(current_csv_file_path, 'w', newline='') as current_csv_file:
                        ...
                        # Perform the scan for this cycle with the current frequency offset
                        scanned_data, last_successful_band_index = scan_bands(
                            self, csv_writer, selected_bands, 
                            scan_rbw_segmentation, rbw_config_val, 
                            vbw_config_val, max_hold_time, self.current_freq_offset # Pass the frequency offset
                        ) 
                        
                        # IMPORTANT: Add this check immediately after scan_bands returns
                        if not self.scanning:
                            print("\nScan process finished (interrupted after band scan).")
                            self.after(0, self.update_progress_label, "Scan interrupted by user.")
                            # Plot the last partial data if available
                            if scanned_data:
                                plot_suffix = f"{scan_name}_{rbw_str}_{hold_str}_{offset_str}_{datetime_str}_INTERRUPTED"
                                self.after(0, self.generate_single_scan_plot_and_open_wrapper, scanned_data, plot_suffix, html_plot_path_for_single_scan, False) # Pass False for auto-open
                            break # Exit the while loop for continuous scanning
    ...
    def generate_single_scan_plot_and_open_wrapper(self, scanned_data, plot_title_suffix, output_html_path, auto_open_browser=True):
        ...
        try:
            fig = plot_single_scan_data( # Get figure only
                scanned_data, 
            ...
            if fig:
                # Save the plot using the provided output_html_path
    ...
    def generate_average_plot(self):
        ...
        if not all_historical_dfs_normalized:
            messagebox.showwarning("No Data Found", f"No valid historical scan data CSV files with prefix '{self.scan_name_var.get()}_' and correct format found in '{base_output_dir}' to generate an average plot.")
            return
