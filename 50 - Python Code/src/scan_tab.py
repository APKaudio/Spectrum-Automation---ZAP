...
class ScanTab(ttk.Frame):
    ...
    def _create_widgets(self):
        ...
        session_details_frame.grid_columnconfigure(2, weight=0)
    ...
    def _save_last_used_config(self):
        ...
        # --- ADDED DEBUG LOGIC ---
        # Get the full path that will be used for saving
        config_file_path = os.path.abspath(self.app_instance.CONFIG_FILE)
        self.console_print_func(f"ℹ️ Attempting to save configuration to: {config_file_path}")
        debug_print(f"Attempting to save configuration to: {config_file_path}", file=current_file, function=current_function, console_print_func=self.console_print_func)
