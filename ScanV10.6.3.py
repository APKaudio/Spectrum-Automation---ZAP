...
def plot_multi_trace_data(aggregated_df, plot_title_suffix, include_tv_markers=True, include_gov_markers=True, output_html_path=None):
    """
    ...
    # Ensure Frequency_MHz column exists
    ...
    # Add Range trace (Max-Min difference)
    ...
    all_y_values = pd.concat([
        aggregated_df['Average_dBm'],
        aggregated_df['Median_dBm'],
        aggregated_df['Range_dBm']
    ])

    y_range_min = all_y_values.min()
    # Set max Y-axis to 0 dBm as requested
    y_range_max = 0 # Fixed maximum to 0 dBm
    y_padding = (y_range_max - y_range_min) * 0.1
    y_range_min -= y_padding
    ...
    # Determine X-axis range for marker visibility check
    x_min_data = aggregated_df['Frequency_MHz'].min()
    x_max_data = aggregated_df['Frequency_MHz'].max()
...
class App(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.load_preset_button = tk.Button(preset_files_frame, text="Load Selected Preset", command=self.load_selected_preset, state=tk.DISABLED)
    ...
    def load_selected_preset(self):
        ...
        try:
            if write_safe(self.inst, command):
                # Removed success dialog box: messagebox.showinfo("Preset Loaded", f"Successfully loaded preset: {selected_preset_name}")
    ...
    def _generate_current_cycle_average_csv_and_plot(self): # Renamed function
        ...
        try:
            fig, plot_html_path_return = plot_multi_trace_data(
                ...
                html_filename # Pass the desired full path for the HTML file
            )
    ...
    def generate_average_plot(self):
        ...
        for file_name in os.listdir(base_output_dir):
            ...
            if file_name.endswith(".csv") and file_name.startswith(self.scan_name_var.get() + "_") and "_averaged_cycle.csv" not in file_name:
        ...
        # Concatenate all historical DataFrames vertically
        ...
        # Plotting the historical averaged, median, and range data
        try:
            fig, plot_html_path_return = plot_multi_trace_data(
                aggregated_df,
