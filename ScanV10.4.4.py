...
DEFAULT_SCAN_RBW_HZ = 10000 # Default for the new "Scan RBW" for segmentation
...
def scan_bands(app_instance, csv_writer, selected_bands, scan_rbw_segmentation, rbw_config_val, vbw_config_val, max_hold_time, last_scanned_band_index=0):
    ...
    for i in range(last_scanned_band_index, len(selected_bands)):
        ...
        write_safe(app_instance.inst, f":SENSE:BAND:RES {rbw_config_val}")
        print(f"📏 Set RBW to {rbw_config_val} Hz for scan.")
        ...
        while current_segment_start_freq_hz < band_stop_freq_hz:
            ...
            if max_hold_time > 0:
                for sec_wait in range(int(max_hold_time), 0, -1):
                    ...
                    app_instance.after(0, app_instance.console_output.write, f"\r{display_text}") # \r to overwrite line
                    app_instance.after(0, app_instance.console_output.flush)
                    time.sleep(1)
                app_instance.after(0, app_instance.console_output.write, "✅\n") # Clear the line and add newline
                app_instance.after(0, app_instance.console_output.flush)
...
class App(tk.Tk):
    ...
    def start_scan_thread(self):
        ...
        rbw_config_val = float(self.desired_rbw_var.get())
