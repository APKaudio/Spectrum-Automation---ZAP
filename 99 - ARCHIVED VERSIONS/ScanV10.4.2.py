# Updated ScanV10.4.2.py with new plot options and auto-open functionality

import tkinter as tk
from tkinter import messagebox, scrolledtext, filedialog, TclError # Added filedialog, TclError
import pyvisa
import time
import argparse
import struct
...
DEFAULT_RBW_STEP_SIZE_HZ = 10000 # Corrected default RBW to 10 kHz as per typical usage
...
def scan_bands(app_instance, csv_writer, bands_to_scan, rbw, vbw, max_hold_time):
    ...
    for i, band in enumerate(bands_to_scan):
        ...
        app_instance.after(0, app_instance.update_progress_label, f"Scanning... {current_scan_progress}/{total_segments} bands complete")
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.geometry("1200x750") # Increased height to accommodate new options
    ...
    def create_widgets(self):
        ...
        row_idx += 1
        tk.Label(scan_settings_frame, text="RBW (Hz):").grid(row=row_idx, column=0, sticky=tk.W, pady=2)
        ...
        row_idx += 1
        tk.Label(scan_settings_frame, text="VBW (Hz):").grid(row=row_idx, column=0, sticky=tk.W, pady=2)
    ...
    def _run_scan(self, csv_writer, max_hold_time, selected_bands):
        ...
        try:
            ...
            rbw_val = float(self.desired_rbw_var.get())
            # Use the displayed VBW value, which is derived from RBW
            vbw_val = float(self.desired_vbw_display_var.get()) 
