    ...
    def generate_average_plot(self):
        ...
        for i, df in enumerate(all_dfs):
            if len(df) == len(reference_freqs_mhz) and np.allclose(df['Frequency_MHz'].values, reference_freqs_mhz):
                ...
                print(f"Warning: Frequencies in scan {i+1} ({len(df)} points) from '{os.path.basename(all_dfs[i].name if hasattr(all_dfs[i], 'name') else 'unknown')}' do not perfectly match reference ({len(reference_freqs_mhz)} points). It will be plotted individually but excluded from simple average calculation.")
