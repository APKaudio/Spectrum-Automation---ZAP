...
# REMOVED: Top-level imports for plotting functions to break circular dependency
# from src.plot_logic import plot_single_scan_data
...
def run_scan_logic(app_instance): # Renamed from _run_scan to run_scan_logic
    ...
    # DEFERRED IMPORTS: Import plotting functions only when run_scan_logic is called
    # This helps break potential circular dependencies
    try:
        from src.plot_logic import plot_single_scan_data
        from utils.averaging_utils import generate_current_cycle_average_csv_and_plot as generate_average_plot_logic
    except ImportError as e:
        app_instance.after(0, app_instance._update_console_line, f"🚨 Critical: Failed to import plotting modules: {e}\n")
        app_instance.after(0, lambda: messagebox.showerror("Import Error", f"Failed to load plotting modules. Please check your Python environment and file paths: {e}"))
        debug_print(f"Critical ImportError in run_scan_logic: {e}", file=file, function=function)
        return # Cannot proceed without plotting functions
