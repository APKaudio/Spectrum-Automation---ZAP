...
def scan_bands(inst, csv_writer, max_hold_time, rbw, last_scanned_band_index=0):
    ...
    for i in range(last_scanned_band_index, len(SCAN_BAND_RANGES)):
        ...
        while current_segment_start_freq_hz < band_stop_freq_hz:
            ...
            if max_hold_time > 0:
                print(f"⏳ Waiting {max_hold_time} seconds for MAX hold to settle...", end='') # Moved emoji
                for sec_wait in range(int(max_hold_time), 0, -1):
                    ...
                    time.sleep(1)
                print("✅ MAX hold settle time complete.                              ") # Moved emoji, Clear the line after countdown
            ...
            print(f"{progressbar} 🔍 Scanning 📈 {current_segment_start_freq_hz/MHZ_TO_HZ:.3f} MHz to {segment_stop_freq_hz/MHZ_TO_HZ:.3f} MHz 📉 (Span: {actual_segment_span_hz/MHZ_TO_HZ:.3f} MHz) 📊 ✨ completed {segment_counter} of {total_segments_in_band}.")
