...
DEFAULT_RBW_STEP_SIZE_HZ = 100000 # Corrected default RBW to 10 kHz as per typical usage
...
DEFAULT_SCAN_RBW_HZ = 100000 # Changed default to 100 kHz as per user request for "Scan RBW" for segmentation
...
def scan_bands(app_instance, csv_writer, selected_bands, scan_rbw_segmentation, rbw_config_val, vbw_config_val, max_hold_time, last_scanned_band_index=0):
    ...
    for i in range(last_scanned_band_index, len(selected_bands)):
        ...
        # --- "Wake Up" and Initial Band Configuration ---
        ...
        # Apply RBW and VBW settings here AFTER the 1kHz wakeup configuration
        ...
        print(f"📏 Set RBW to {scan_rbw_segmentation} Hz for scan (from Scan RBW setting).")
        write_safe(app_instance.inst, f":SENSE:BAND:VID {vbw_config_val}")
        print(f"📺 Set VBW to {vbw_config_val} Hz for scan.")
        #query_safe(app_instance.inst, "*OPC?") # Wait for operations to complete
        #app_instance.inst.clear() # Flush buffer after OPC query


        # Query the actual number of sweep points from the instrument for *this band*
        actual_sweep_points = 461 # Hardcoded as per user request
        ...
        while current_segment_start_freq_hz < band_stop_freq_hz:
            ...
            if max_hold_time > 0:
                # The user's request for the countdown was:
                ...
                # sys.stdout.flush()
                #
                # Adapting this for Tkinter's console_output:
                for sec_wait in range(int(max_hold_time), 0, -1):
                    display_text = f"⏳ Waiting for Max Hold: {sec_wait}s "
                    # Use app_instance.after to update GUI from the thread safely
                    app_instance.after(0, app_instance.console_output.write, f"\r{display_text}")
                    app_instance.after(0, app_instance.console_output.flush)
                    time.sleep(1)
                app_instance.after(0, app_instance.console_output.write, "\r✅ Max Hold wait complete.\n") # Clear line and add newline
                app_instance.after(0, app_instance.console_output.flush)
...
def plot_data(scanned_data, csv_file_path, include_tv=True, include_gov=True):
    """
    ...
    # If scan_bands returns list of dictionaries, convert it here if needed.
    # Current scan_bands returns list of tuples for plot_data compatibility.
    ...
    # Add vertical rectangles for TV bands (conditional)
    if include_tv:
    ...
    # Add vertical rectangles for Government bands (conditional)
    if include_gov:
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.geometry("1200x780") # Increased height to accommodate new options
    ...
    def create_widgets(self):
        ...
        row_idx += 1
        tk.Label(scan_settings_frame, text="Open HTML Plot After Scan:").grid(row=row_idx, column=0, sticky=tk.W, pady=2)
        ...
        try:
            ...
            if instrument_id:
                ...
                if initialize_instrument(self.inst, ref_level, preamp_on, display_log, rbw_config, vbw_config, max_hold_on):
                    ...
                    self.apply_button.config(state=tk.DISABLED) # Disable apply if initialization fails
    ...
    def disconnect_instrument(self):
        ...
        if self.inst:
            try:
                ...
                # self.current_sweep_points_var.set("N/A") # This variable was removed
    ...
    def start_scan_thread(self):
        ...
        vbw_config_val = float(self.desired_vbw_display_var.get()) # Use the displayed VBW
        ...
        try:
            ...
            # Write CSV header
            csv_writer.writerow(["Frequency (Hz)", "Level (dBm)"]) # Changed to Hz for consistency
            ...
            scan_thread = threading.Thread(target=self._run_scan, 
                                           args=(csv_writer, selected_bands, 
    ...
    def _run_scan(self, csv_writer, selected_bands, scan_rbw_segmentation, rbw_config_val, vbw_config_val, max_hold_time):
        """Internal method to run the scan logic, called by the thread."""
        try:
            # The scan_bands function now handles instrument configuration internally
            ...
            if not self.scanning: # If scan was stopped by user
                ...
                self.after(0, self.update_progress_label, "Scan interrupted by user.") # Update on main thread
            else:
                print("\nScan process finished.")
                self.after(0, self.update_progress_label, "Scan complete.") # Update on main thread
                
                # --- New: Auto-plot after scan completes ---
                if self.open_html_after_complete_var.get():
                    self.after(0, self.generate_plot_and_open) # Schedule plot generation on main thread
                # --- End New ---
            
        except Exception as e:
            # Ensure any error messages are displayed on the main thread
            self.after(0, messagebox.showerror, "Scan Error", f"An error occurred during scanning: {e}")
            print(f"❌ Scan thread encountered an error: {e}")
            self.after(0, self.update_progress_label, f"Scan error: {e}") # Update on main thread
        finally:
            self.scanning = False
    ...
    def reset_scan_buttons(self):
        ...
        if self.last_scan_data: # Only show plot button if there's data
            self.plot_button.pack(side=tk.LEFT, padx=5, pady=5, expand=True, fill=tk.X) # Show plot button after scan finishes
        else:
            self.plot_button.pack_forget() # Hide if no data
    ...
    def generate_plot(self):
        ...
        try:
            # Pass the boolean values from the Tkinter variables
            plot_path = plot_data(
                self.last_scan_data, 
            ...
            return plot_path # Return the path so generate_plot_and_open can use it
        except Exception as e:
            messagebox.showerror("Plot Error", f"Failed to generate plot: {e}")
            print(f"❌ Error generating plot: {e}")
            return None # Return None on error

    def generate_plot_and_open(self):
        """Generates the plot and optionally opens it in a browser."""
        plot_path = self.generate_plot() # Call the existing generate_plot method
        if plot_path and self.open_html_after_complete_var.get():
            try:
