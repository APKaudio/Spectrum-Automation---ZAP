...
DEFAULT_FREQ_SHIFT_HZ = 1000 # New: Default Frequency Shift value
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.shift_freq_var = tk.StringVar(self, value=str(DEFAULT_FREQ_SHIFT_HZ))
    ...
    def create_widgets(self):
        ...
        entry_shift_freq = tk.Entry(scan_settings_frame, textvariable=self.shift_freq_var, bg="grey", fg="white", insertbackground="white")
        entry_shift_freq.grid(row=row_idx, column=1, sticky=tk.EW, pady=2)
        self.desired_setting_entries["shift_freq"] = entry_shift_freq
    ...
    def generate_average_plot(self):
        ...
        # Regex to extract datetime string from filename: e.g., MyScan_RBW####K_HOLD##_Offset####_YYYYMMDD_HHMMSS.csv
        filename_pattern = re.compile(r'^(.*)_RBW(\d{4}K)_HOLD(\d{2})_Offset(\d+)_(\d{8}_\d{6})\.csv$')

        reference_frequencies = None
        reference_num_rows = -1
            ...
            if file_name.endswith(".csv") and file_name.startswith(self.scan_name_var.get() + "_") and "_averaged_cycle.csv" not in file_name and "_HISTORICAL_" not in file_name:
                ...
                try:
                    ...
                    if "Frequency (MHz)" in df.columns and "Level (dBm)" in df.columns:
                        ...
                        current_frequencies = set(df['Frequency_MHz'].unique())
                        current_num_rows = len(df)

                        if reference_frequencies is None:
                            # Set the first valid file as the reference
                            reference_frequencies = current_frequencies
                            reference_num_rows = current_num_rows
                            print(f"Set reference for historical average from '{file_name}': {reference_num_rows} rows, {len(reference_frequencies)} unique frequencies.")
                        
                        # Omit files with different lengths or different frequency sets
                        if current_num_rows != reference_num_rows or current_frequencies != reference_frequencies:
                            print(f"Skipping '{file_name}': Different length ({current_num_rows} rows vs expected {reference_num_rows}) or different frequency points. Omitting from average.")
                            continue # Skip this file
                        
                        # If it passes the check, append to list for aggregation
                        all_historical_dfs.append(df[['Frequency_MHz', 'Power_dBm']].copy())

                        # Extract components for layer name
                        match = filename_pattern.match(file_name)
                        if match:
                            scan_name_prefix, rbw_val, hold_val, offset_val, datetime_val = match.groups()
                            # Format datetime for display
                            display_datetime = datetime.strptime(datetime_val, "%Y%m%d_%H%M%S").strftime("%Y-%m-%d %H:%M:%S")
                            display_name = f"{scan_name_prefix}_{rbw_val}_{hold_val}_Offset{offset_val} ({display_datetime})"
        ...
        if not all_historical_dfs:
            messagebox.showwarning("No Data Found", f"No valid historical scan data CSV files with prefix '{self.scan_name_var.get()}_' and consistent length/frequencies found in '{base_output_dir}' to generate an average plot.")
            return
