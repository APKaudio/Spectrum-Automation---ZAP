...
def scan_bands(inst, csv_writer, max_hold_time, rbw, selected_bands, last_scanned_band_index=0):
    ...
    # Set trace format to ASCII
    write_safe(inst, ":TRAC:FORMat ASCii")
    print("💾 Set trace data format to ASCII for efficient data transfer.") # Moved emoji
    ...
    for i in range(last_scanned_band_index, len(selected_bands)):
        ...
        while current_segment_start_freq_hz < band_stop_freq_hz:
            ...
            try:
                ...
                # Removed redundant inst.write(":TRAC1:DATA?")
                ...
                for j, amp_value in enumerate(trace_data):
                    # Corrected variable name from freq_per_point_actual to freq_step_per_point_actual
                ...
                print(f"🐛 Raw data string potentially causing error: {raw_data_string[:100]}...") # Moved emoji
                ...
                print(f"🚫 Error processing ASCII data (ValueError - cannot convert to float): {e}") # Moved emoji
                print(f"🐞 Raw data string for parsing: {raw_data_string[:100]}...") # Moved emoji
            except Exception as e:
...
class ScanApp:
    ...
    def create_widgets(self):
        # --- Scan Name ---
        tk.Label(self.master, text="Scan Series Name:").pack(pady=(10, 2))
        self.scan_name_entry = tk.Entry(self.master, textvariable=self.scan_name_var, width=50)
        self.scan_name_entry.pack(pady=2)

        # --- RBW Step Size ---
        tk.Label(self.master, text="RBW Step Size (Hz):").pack(pady=(10, 2))
        self.rbw_step_size_entry = tk.Entry(self.master, textvariable=self.rbw_step_size_var, width=20)
        self.rbw_step_size_entry.pack(pady=2)

        # --- Max Hold Time ---
        tk.Label(self.master, text="Max Hold Time (seconds):").pack(pady=(10, 2))
        self.max_hold_time_entry = tk.Entry(self.master, textvariable=self.max_hold_time_var, width=20)
        self.max_hold_time_entry.pack(pady=2)

        # --- Cycle Wait Time ---
        tk.Label(self.master, text="Cycle Wait Time (seconds):").pack(pady=(10, 2))
        self.cycle_wait_time_entry = tk.Entry(self.master, textvariable=self.cycle_wait_time_var, width=20)
        self.cycle_wait_time_entry.pack(pady=2)
