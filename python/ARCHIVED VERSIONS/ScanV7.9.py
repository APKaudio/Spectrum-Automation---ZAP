...
DEFAULT_RBW_STEP_SIZE_HZ = 20000 # 10 kHz RBW resolution desired per data point
DEFAULT_CYCLE_WAIT_TIME_SECONDS = 300 # 5 minutes wait (300 seconds) between full scan cycles
DEFAULT_MAXHOLD_TIME_SECONDS = 4 # Default max hold time for the new argument
...
def setup_arguments():
    ...
    parser.add_argument('--name', type=str, default="Scan", # Changed default to "Scan" to be consistent
                        help='Prefix for the output CSV filename and subfolder name if used.')
        ...
        try:
            ...
            try:
                if inst:
                    ...
                    print("Sent instrument restart command. Waiting for it to come back online...")
                    ...
                    print("Instrument not initialized yet, skipping direct restart command. Will attempt full initialization.")
            except Exception as e:
                print(f"Could not send restart command (likely connection already down or instrument rebooting): {e}. Proceeding to re-initialize.")
            ...
            print(f"Re-attempting full connection and IDN query after potential restart (Attempt {attempt + 1}/{max_retries})...")
            ...
            if "[Not Supported or Timeout]" not in idn_response and idn_response:
                print(f"\nSuccessfully connected to: {idn_response}\n")
                ...
                print("Display set to reference level -30 dBm.")
            ...
            #        write_safe(inst, f":CALC:MARK{i}:MODE POS")
             #       print(f"Marker {i} enabled in position mode (for peak table display).")
                print("Trace 1 set to max hold")
                write_safe(inst, ":TRAC1:MODE MAXHold")
                
                return inst
            else:
                print(f"  Failed to get IDN response: {idn_response}. Retrying...")
            ...
            print(f"An unexpected error occurred during instrument initialization (Attempt {attempt + 1}): {e}")
            print(f"Retrying in {retry_delay} seconds...")
            if inst:
                inst.close()
            time.sleep(retry_delay)
...
def scan_bands(inst, csv_writer, max_hold_time, rbw, last_scanned_band_index=0):
    ...
    for i in range(last_scanned_band_index, len(SCAN_BAND_RANGES)):
        ...
        # --- NEW: "Wake Up" and Initial Band Configuration ---
        ...
        # --- END NEW "Wake Up" and Initial Band Configuration ---
...
def plot_spectrum_data(df: pd.DataFrame, output_html_filename: str, include_gov_markers: bool, include_tv_markers: bool):
    """
...
def wait_with_interrupt(wait_time_seconds):
    ...
    if not skip_wait:
        ...
        sys.stdout.write("Starting next scan now!\n") # Print a full message after countdown
        sys.stdout.flush()
...
def main():
    ...
    try:
        while True: # This loop makes the program repeat indefinitely
            ...
            # Create subfolder name based on --name argument
            # Clean up the name for path safety: remove leading/trailing spaces and replace internal spaces with underscores
            name_folder = args.name.strip().replace(" ", "_")
            
            # Ensure the name_folder is not empty or just underscores if --name was provided but empty/whitespace
            if not name_folder or name_folder == "_":
                name_folder = "Default_Scan_Data" # Provide a sensible default if --name is empty or just spaces
            ...
            try:
                ...
                if not all_scan_data_current_cycle:
                    ...
                    # Call the plotting function, passing the new marker arguments
                    plot_spectrum_data(df, html_plot_filename, args.gov, args.tv)
