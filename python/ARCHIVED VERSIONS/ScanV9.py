...
def initialize_instrument(inst, clear_reset, preamplifier_on, display_log, ref_level, max_hold_on, rbw_config_val):
    ...
    try:
        ...
        print(f"✅ Markers 1-4 enabled and set to {rbw_config_val} bandwidth.")
...
def scan_bands(inst, csv_writer, max_hold_time, rbw, selected_bands, last_scanned_band_index=0, rbw_config_val="1KHZ", vbw_config_val="1KHZ"):
    ...
    for i in range(last_scanned_band_index, len(selected_bands)):
        ...
        while current_segment_start_freq_hz < band_stop_freq_hz:
            ...
            marker_offsets_percentage = [0.0, 0.25, 0.50, 0.75] # Relative to current segment span
            ...
            while (current_marker_base_freq + (marker_offsets_percentage[3] * actual_segment_span_hz)) <= segment_stop_freq_hz:
                try:
                    marker_data_points_temp = [] # Temporary list to store data before appending to all_scan_data
                    
                    # First loop: Set all marker frequencies
                    for marker_idx in range(4): # For markers 1, 2, 3, 4
                        marker_num = marker_idx + 1
                        
                        # Calculate the frequency for the current marker
                        # It's base frequency + offset based on segment span
                        marker_freq_hz = current_marker_base_freq + (marker_offsets_percentage[marker_idx] * actual_segment_span_hz)
                        
                        # Ensure marker frequency does not exceed the segment's stop frequency
                        marker_freq_hz = min(marker_freq_hz, segment_stop_freq_hz)

                        # Set marker frequency - NO OPC
                        write_safe(inst, f":CALC:MARK{marker_num}:X {marker_freq_hz}HZ")
                    
                    # Second loop: Query all marker amplitudes
                    for marker_idx in range(4): # For markers 1, 2, 3, 4
                        marker_num = marker_idx + 1
                        
                        # Query amplitude - NO OPC, NO inst.clear()
                        amp_value_str = inst.query(f":CALC:MARK{marker_num}:Y?").strip()
                        
                        amp_value = float(amp_value_str)

                        # Re-calculate marker_freq_hz for data point to ensure consistency
                        marker_freq_hz = current_marker_base_freq + (marker_offsets_percentage[marker_idx] * actual_segment_span_hz)
                        marker_freq_hz = min(marker_freq_hz, segment_stop_freq_hz)
