...
def initialize_instrument(inst, clear_reset, preamplifier_on, display_log, ref_level, max_hold_on):
    """
...
def scan_bands(inst, csv_writer, max_hold_time, rbw, selected_bands, last_scanned_band_index=0, rbw_config_val="1KHZ", vbw_config_val="1KHZ"):
    ...
    # Removed: write_safe(inst, ":FORMat:DATA REAL,32")
    # As per user feedback, the instrument might be sending ASCII data despite this command.
    # We will now read the data as ASCII and parse it.
    print("💾 Assuming ASCII data format for trace data based on previous error.")

    # For binary data, read_termination and encoding are less critical as query_binary_values
    # handles the block data format directly. However, it's good practice to ensure
    # they are not set to something that might interfere. PyVISA's default for binary
    # should be fine, but we'll explicitly clear/reset them if needed.
    # inst.read_termination = None # Or set to a default that doesn't interfere
    # inst.encoding = 'latin-1' # Or whatever default PyVISA uses for binary
    ...
    for i in range(last_scanned_band_index, len(selected_bands)):
        ...
        write_safe(inst, f":SENSE:BAND:RES {rbw_config_val}")
        print(f"📏 Set RBW to {rbw_config_val} for scan.")
        write_safe(inst, f":SENSE:BAND:VID {vbw_config_val}")
        print(f"📺 Set VBW to {vbw_config_val} for scan.")
        query_safe(inst, "*OPC?") # Wait for operations to complete
        ...
        actual_sweep_points = 462 # Hardcoded as per user request
        ...
        while current_segment_start_freq_hz < band_stop_freq_hz:
            ...
            try:
                ...
                if "[Not Supported or Timeout]" in trace_data_str or not trace_data_str:
                ...
                if num_trace_points_actual > 1:
                    ...
                    print("🚫 No trace data received for this segment after parsing.") # Moved emoji
                    current_segment_start_freq_hz = segment_stop_freq_hz
                    continue # Move to the next segment if no data
                ...
                print(f"🚫 Error processing ASCII trace data (ValueError - cannot convert/unpack): {e}") # Moved emoji
                print(f"🐞 Raw data string for parsing: {e}") # Print the error message for debugging
            except Exception as e:
...
class ScanApp:
    def __init__(self, master):
        ...
        master.geometry("700x850") # Adjusted width to 700 pixels
    ...
    def create_widgets(self):
        ...
        self.start_scan_button = tk.Button(button_row_frame, text="Start Scan", command=self.start_scan, state=tk.DISABLED, bg="green")
        ...
        self.max_hold_time_entry = tk.Entry(scan_params_frame, textvariable=self.max_hold_time_var, width=20)
        ...
        if available_resources:
            ...
            try:
                ...
                if initialize_instrument(
                    ...
                    # RBW and VBW are no longer set here
        ...
        while True: # This loop makes the program repeat indefinitely
            ...
            try:
                ...
                if not initialize_instrument(
                    ...
                    # rbw_config_val, # Removed from here, now set in scan_bands
                    # vbw_config_val, # Removed from here, now set in scan_bands
