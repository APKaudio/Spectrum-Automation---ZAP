...
def plot_spectrum_data(df, output_html_filename):
    ...
    # Define a list of colors for the background bands. You can customize these.
    # Using subtle, semi-transparent colors.
    band_colors = [
        "rgba(255, 0, 0, 0.05)",    # Light red
        "rgba(0, 0, 255, 0.05)",    # Light blue
        "rgba(0, 255, 0, 0.05)",    # Light green
        "rgba(255, 255, 0, 0.05)",  # Light yellow
        "rgba(255, 0, 255, 0.05)",  # Light magenta
        "rgba(0, 255, 255, 0.05)",  # Light cyan
    ]
    color_index = 0

    # Add background shapes for each frequency band
    # First, determine the full Y-axis range to ensure shapes cover the entire plot height
    y_min = df['Level (dBm)'].min()
    y_range_min = y_min if y_min < -100 else -100 # Ensure a sensible minimum if data is higher
    y_range_max = 0 # As per your existing update_yaxes setting
    ...
    for band in BAND_RANGES:
        fig.add_shape(
            ...
            x1=band["Stop MHz"],
            y1=y_range_max,
            fillcolor=band_colors[color_index % len(band_colors)],
            opacity=0.2, # Adjust transparency as needed
            layer="below", # Crucial: draw below the trace lines
            line_width=0, # No border around the rectangles
        )
        color_index += 1
    ...
    # Set Y-axis (Amplitude) Maximum to 0 dBm (re-apply, add_shape might override some layout)
