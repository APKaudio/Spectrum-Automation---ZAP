...
class App(tk.Tk):
    ...
    def create_widgets(self):
        ...
        # Removed Instrument RBW and VBW from UI as requested
        # row_idx += 1
        ...
        button_row_frame = tk.Frame(scan_settings_frame)
        ...
        self.apply_button = tk.Button(button_row_frame, text="Apply Settings to Device", command=lambda: self.apply_settings_to_device(), state=tk.DISABLED)
    ...
    def generate_average_plot(self):
        ...
        for file_name in os.listdir(base_output_dir):
            ...
            if file_name.endswith(".csv") and file_name.startswith(self.scan_name_var.get() + "_") and "_averaged_cycle.csv" not in file_name and "_HISTORICAL_" not in file_name:
                ...
                try:
                    # Fix for SettingWithCopyWarning: Use .copy() to ensure a new DataFrame is created
                    ...
                    if "Frequency (Hz)" in df.columns and "Level (dBm)" in df.columns:
                        ...
                        all_historical_dfs.append(df[['Frequency_Hz', 'Power_dBm']].copy()) # FIX: Explicitly copy slice
                        ...
                        if match:
                            ...
                            historical_dfs_for_overlays.append((df[['Frequency_Hz', 'Power_dBm']].copy(), display_name)) # FIX: Explicitly copy slice
                        else:
                            historical_dfs_for_overlays.append((df[['Frequency_Hz', 'Power_dBm']].copy(), file_name)) # Fallback to full filename # FIX: Explicitly copy slice
